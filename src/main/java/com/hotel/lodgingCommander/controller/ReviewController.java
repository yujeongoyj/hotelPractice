...
import com.hotel.lodgingCommander.service.ReivewService;
import com.hotel.lodgingCommander.service.impl.ReviewImpl;
...
public class ReviewController {
    ...
    public ReviewController(ReivewService reivewService) {
    ...
    public ResponseEntity<Map<String, Object>> addReview(@RequestBody ReviewDTO reviewDTO) {
        ...
        try {
            // 리뷰 생성 서비스 호출
            ReviewDTO createdReview = reivewService.createReview(reviewDTO);
    ...
    public ResponseEntity<Map<String, Object>> findReviewsByUserId(@PathVariable Long userId) {
        Map<String, Object> resultMap = new HashMap<>();
        try {
            List<ReviewDTO> reviews = reivewService.findReviewsByUserId(userId);
    ...
    public ResponseEntity<Map<String, Object>> findReviewsByHotelId(@PathVariable Long hotelId) {
        Map<String, Object> resultMap = new HashMap<>();
        try {
            List<ReviewDTO> reviews = reivewService.findReviewsByHotelId(hotelId);
            resultMap.put("reviews", reviews);
            return ResponseEntity.ok(resultMap);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ResponseEntity<Map<String, Object>> updateReview(@PathVariable Long id, @RequestBody ReviewDTO reviewDTO) {
        Map<String, Object> resultMap = new HashMap<>();
        try {
            ReviewDTO updatedReview = reivewService.updateReview(id, reviewDTO);
